...
// Stiva de liste (folosita la task 2)
...
typedef struct NodArbore 
{
    ...
    char matrice[MAX_LINII][MAX_COLOANE]; // matricea asociata nodului  // trebuiau puse doar coordonatele, dupa cateva incercari m-am dat batut
...
void task1(FILE* in, FILE* out) 
{
 ...
 void afiseaza_modificari_ordine(NodStiva* nod, FILE* out, int generatia_curenta) {
        if (!nod) return;
        afiseaza_modificari_ordine(nod->urmator, out, generatia_curenta - 1);

        fprintf(out, "%d", generatia_curenta);
        for (Celula* c = nod->lista->inceput; c; c = c->urmator) {
            fprintf(out, " %d %d", c->linie, c->coloana);
        }
        fprintf(out, "\n");
    }
...
void task2(FILE* in, FILE* out) {
    ...
    for (int g = 0; g < k; g++) {
        ...
        Lista* modificari = diferenta_matrici(matrice, urmatoarea_matrice, n, m);
        push_stiva(&stiva, modificari);
        copie_matrice(urmatoarea_matrice, matrice, n, m);
    }

    // Afiseaza modificarile
    afiseaza_modificari_ordine(stiva, out, k);
} 
...
int vizitat[MAX_LINII][MAX_COLOANE]; // matrice pentru marcarea celulelor vizitate
Varf drum_maxim[MAX_LINII * MAX_COLOANE]; // retine drumul maxim gasit
Varf drum_temp[MAX_LINII * MAX_COLOANE];  // retine drumul curent in DFS
...
void determina_lant_hamiltonian(FILE* out, char matrice[MAX_LINII][MAX_COLOANE], int n, int m) 
{
    n_global = n;
    m_global = m;
    lungime_maxima = -1;
    ...
    for (int i = 0; i < n; i++) for (int j = 0; j < m; j++) 
    {
        if (matrice[i][j] == 'X') 
        {
            ...
            dfs_hamiltonian(matrice, i, j, 0, 0, 0);
        }
    }
...
NodStiva* citeste_stiva(FILE* in, char matrice[MAX_LINII][MAX_COLOANE], int* n, int* m, int* k) {
    fscanf(in, "%d %d", n, m);
    for (int i = 0; i < *n; i++) {
        fscanf(in, "%s", matrice[i]);
    }
    ...
    for (int gen = 0; gen < *k; gen++) {
        ...
        for (int i = 0; i < nr_modificari; i++) {
            int l, c;
            fscanf(in, "%d %d", &l, &c);
            adauga_celula(lista, l, c);
        }

        push_stiva(&stiva, lista);
    }
...
void bonus(FILE* in, FILE* out) {
    char matrice[MAX_LINII][MAX_COLOANE];
    int n, m, k;
    NodStiva* stiva = citeste_stiva(in, matrice, &n, &m, &k);

    while (stiva) {
        undo(matrice, &stiva);
    }

    fprintf(out, "Generatia 0 restaurata:\n");
